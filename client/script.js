...
sayHelloButton.addEventListener('mouseover', colorOn)
...
const colorOff = evt => {
    ...
    target.style.backgroundColor = '#EFEFEFEF';
    target.style.color = 'black';
}

sayHelloButton.addEventListener('mouseout', colorOff)
...
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/'wake up!'`).then((res) => {
...
const queryRequest = () => {
    axios.get(`http://localhost:3000/query-test/?[How old are you?] ?[41, 41]`).then((res) => {
        let repeatText = document.getElementById('repeat-text');
        repeatText.style.display = 'block';
        repeatText.textContent = res.data;
    });
